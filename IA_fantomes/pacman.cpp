...
void Pacman::deplacementPacman(Mur ensembleMurs, sf::Clock clock, const TunnelHorizontal& t, int TAILLE_ENTITE, int SPEED, char &derniereCase, float AGRANDISSEMENT){
    ...
    int posX =_pacman.getBody().getRect().getPosition().x;
    int posY = _pacman.getBody().getRect().getPosition().y;
    ...
    _pacman.interactionTunnelHorizontaux(t, _spritePacman, TAILLE_ENTITE/AGRANDISSEMENT, AGRANDISSEMENT);
    }
}
